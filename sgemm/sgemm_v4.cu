    ...
    for (int t = 0; t < TILE_CNT; ++t) {  // 分块加载 
        // shared_a: transpose
        {
            ...
            if (full_a) {
                ...
                for (int j = 0; j < 4; ++j) {
                    shared_a[s_col + j][s_row] = (g_row < M && g_col + j < K) ? A(g_row, g_col + j) : 0.0f;
                }
            }
        }
        // shared_b
...
void launch_sgemm_v4(float *a, float *b, float *c) {
    ...
    dim3 block{N_THREAD_PER_BLOCK, M_THREAD_PER_BLOCK};  // float4 加载，每线程加载 1 float4, 计算 4*4
...
int main() {
    ...
    launch_sgemm_v4(mA_device, mB_device, mC_device);
